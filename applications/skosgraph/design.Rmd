...
* Each thesaurus has its own namespace so that concepts with the same name can co-exist. 
* The base namespace is litrev, but this is not for instances but mainly for meta-information. 
* In the litrev namespace there must be a description where to find information about the thesauri. Currently I do this as meta-data on the predicate description:
...
Based on this, one can make a connection with descriptions of a thesaurus, which consists mainly of SKOS statements, with a bit of additional information:

```
lo:OutcomesThesaurus a skos:ConceptScheme ; 
...
The connection can be made with queries like this one: 
...
```
This way, the thesauri can stay in their individual namespace, which does not have to be known to the app beforehand. 

* Instances (bib references etc can live in any namespace, like ones for the literature review project/study.)

## Transform cito cites properties into skos
