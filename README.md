...
The second main concept is the *semantic literature review*: LitNet builds on RDF and SKOS--semantic web standards--and uses a RDF database. Semantic methods allow to express more complex relations than attribute-value pairs and can do so more easily than using relational databases. In effect, LitNet expresses bibliographic and coded information as a [Knowledge Graph](http://www.google.com).
